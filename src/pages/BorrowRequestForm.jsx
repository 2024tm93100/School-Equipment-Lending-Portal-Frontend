...
const BorrowRequestForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    startDate: "",
    endDate: "",
    userId: localStorage.getItem("id") ? parseInt(localStorage.getItem("id")) : null,
    status: "PENDING",
  });
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "requestedQuantity" || name === "equipmentId"
          ? parseInt(value) || 1
          : value,
    }));
  };
    ...
    try {
      ...
      setError(
        `Failed to submit request: ${
          err.response?.data?.message || "Check quantity or dates."
        }`
      );
    } finally {
      setIsSubmitting(false);
  ...
  if (loading)
    return (
      <div className="text-center mt-5">Loading available equipment...</div>
    );
  if (role !== "STUDENT")
    return (
      <div className="alert alert-danger mt-5">
        Access Denied. Only Students can submit requests.
      </div>
    );
  ...
  if (availableEquipment.length === 0) {
    return (
      <div className="alert alert-info mt-5">
        No equipment is currently available for borrowing.
      </div>
    );
  }
          ...
          >
            {availableEquipment.map((item) => (
              <option key={item.equipmentId} value={item.equipmentId}>
                {item.name} ({item.availableQuantity} available, Condition:{" "}
                {item.condition})
              </option>
            ))}
          </select>
          <small className="form-text text-muted">
            Only items with stock are shown.
          </small>
        </div>

        {/* Quantity */}
        <div className="mb-3">
          <label htmlFor="requestedQuantity" className="form-label">
            Quantity
          </label>
          <input
            type="number"
            ...
            max={
              availableEquipment.find(
                (e) => e.equipmentId === formData.equipmentId
              )?.availableQuantity || 1
            }
          />
        </div>
